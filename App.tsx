    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300 overflow-hidden font-sans">
      ...
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} hidden md:flex flex-col bg-slate-900 dark:bg-black text-white transition-all duration-300 z-50 shrink-0 shadow-2xl`}>
        <div className="p-6 flex items-center justify-between">
          {isSidebarOpen ? <h1 className="text-xl font-black text-blue-400 tracking-tighter uppercase">MS APP</h1> : <ShieldCheck className="text-blue-400" />}
        ...
        <nav className="flex-1 mt-4 overflow-y-auto custom-scrollbar">
          {!isOperacional && <SidebarItem to="/" icon={<LayoutDashboard size={22} />} label="Painel" isOpen={isSidebarOpen} active={location.pathname === '/'} />}
          <SidebarItem to="/qr-scan" icon={<QrCode size={22} />} label="Scanner" isOpen={isSidebarOpen} active={location.pathname === '/qr-scan'} />
          <SidebarItem to="/condos" icon={<Building2 size={22} />} label="Vistorias" isOpen={isSidebarOpen} active={location.pathname.includes('/condos')} />
          ...
          {!isOperacional && <SidebarItem to="/reports" icon={<FileText size={22} />} label="Relatórios" isOpen={isSidebarOpen} active={location.pathname === '/reports'} />}
        </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 z-40 shrink-0 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-100 dark:bg-blue-900/40 rounded-lg text-blue-600 dark:text-blue-400"><UserIcon size={20} /></div>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 pb-32 md:pb-8 bg-slate-50 dark:bg-slate-950">
          <Routes>
            ...
            <Route path="/reports" element={!isOperacional ? <Reports /> : <Navigate to="/" />} />
            <Route path="*" element={<Navigate to="/" />} />
          </Routes>
        </main>

        {/* Mobile Nav - Fixed Z-Index and functionality */}
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex justify-around items-center p-3 z-[100] shadow-[0_-4px_15px_rgba(0,0,0,0.1)]">
          {!isOperacional && (
          ...
          <button onClick={() => navigate('/condos')} className={`p-3 rounded-2xl transition-all ${location.pathname.includes('/condos') || location.pathname.includes('/checklist') ? 'text-blue-600 bg-blue-50 dark:bg-blue-900/30' : 'text-slate-400'}`}>
            <ListChecks size={26} />
          </button>
          ...
          <button onClick={() => logout()} className="p-3 text-red-500 active:scale-75 transition-all">
            <LogOut size={26} />
          </button>
        </div>
      </div>
...
const App: React.FC = () => {
  ...
  const login = (role: UserRole) => {
    const newUser = { 
      ...
      name: role === UserRole.OPERACIONAL ? 'Ricardo' : (role === UserRole.SINDICO ? 'Ana Síndica' : 'Admin Central'), 
      role 
    };
    setUser(newUser);
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 font-sans">
        <div className="max-w-md w-full bg-white dark:bg-slate-800 rounded-[2.5rem] p-10 text-center shadow-2xl border border-slate-200 dark:border-slate-700">
          <div className="w-24 h-24 bg-blue-50 dark:bg-blue-900/30 rounded-3xl flex items-center justify-center mx-auto mb-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-10 font-medium uppercase tracking-widest text-xs">Gestão de Vistoria Predial</p>
          <div className="space-y-4">
            ...
            <button onClick={() => login(UserRole.OPERACIONAL)} className="w-full py-4.5 border-2 border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 rounded-2xl font-bold hover:bg-slate-50 dark:hover:bg-slate-700 active:scale-95 transition-all cursor-pointer">ZELADOR / OPERADOR</button>
          </div>
        </div>
      </div>
    );
